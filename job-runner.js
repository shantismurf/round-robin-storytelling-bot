...
import { getConfigValue, log, replaceTemplateVariables, discordTimestamp, closeOrphanedGuildStories, isGuildConfigured, createFailureThrottle } from './utilities.js';
...
async function runDueJobs(connection, client) {
  try {
    ...
    const recovery = dbPollThrottle.onSuccess();
    if (recovery) {
      log(`Job runner poll recovered after ${recovery.consecutiveFailures} consecutive failure(s) (~${recovery.downForMin}m down)`, { show: true });
    }
    for (const job of jobs) {
      await processJob(connection, client, job);
    }
  } catch (err) {
    const result = dbPollThrottle.onFailure();
    if (result.isSummary) {
      log(`Job runner poll still failing — ${result.consecutiveFailures} consecutive failures, down for ~${result.downForMin}m. Latest: ${err}`, { show: true });
    } else {
      log(`Job runner poll error: ${err}`, { show: result.show });
    }
  }
}
