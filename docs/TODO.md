...
- **`/story manage`'s Reopen button doesn't refresh `state.targetStatus`/`originalStatus` after reopening** — found 2026-08-22 while building Part 1c's dirty-check (`docs/plans/PLAN-panel-rework-and-ground-rules.md`). `story_manage_reopen` (`handleManageButton`, `story/manage.js`) writes the reopen straight to the DB via `handleReopenStory`, then re-renders the panel from the same `state` object without updating `targetStatus`/`originalStatus` — so the just-reopened panel still shows the "Reopen" button and a disabled Pause/Resume row until it's reopened fresh (`/story manage` again). Pre-existing, unrelated to the dirty-check itself (reopen isn't a staged field, so it doesn't falsely trip the new unsaved-changes warning either way). Fix: set both to `STORY_STATUS.ACTIVE` alongside the DB write in that branch.
