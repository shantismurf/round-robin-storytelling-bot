...
- **[LOW PRIORITY] File-size split pass** — line count audit taken 2026-07-12 after the Fable Audit step 5 session (which touched nearly every file in the codebase). Not urgent; do as a dedicated session whenever it becomes worth it, not opportunistically mid-other-work like the Step 6 folds were. Six files over the 500-line CLAUDE.md standard, in priority order (`edit.js` and `utilities.js` are repeat offenders — both were already flagged in the original May Fable Audit and crept back over 500 despite partial shrinkage since):
  ...
  - **`story/manage.js` (593 lines as of 2026-08-21's Components V2 panel rework — grew from 566, despite removing two dead code paths in the same pass)** — panel build (`buildManageMessage`, `handleManage`) vs. button routing (`handleManageButton`) vs. save/modal-submit (`handleManageSave`, `handleManageModalSubmit`). The save logic alone is substantial; could become `story/_manageSave.js`.
  ...
  - **`story/add.js` (501 lines as of the same rework — was 471)** — crossed the line during the Components V2 panel rework; same shape as manage.js's split candidates (panel build vs. button routing vs. modal-submit) once this pass is picked up.
  - Also worth a light look when doing this pass (currently just under 500, likely to cross it next time they're touched): `story/_manageTurnActions.js` (474), `commands/_myStoryList.js` (465).
