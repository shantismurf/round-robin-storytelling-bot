...
- Story entry mentions (`@user`/`#channel`/`@role`) should always resolve to plain display text, never a raw ID or a functional link, anywhere they're shown — see [plans/PLAN-mention-display-text.md](plans/PLAN-mention-display-text.md) for full plan (surfaced while checking Discord's upcoming channel-obfuscation change against `story/export.js`; design fully resolved with user 2026-08-17, no schema change needed).
