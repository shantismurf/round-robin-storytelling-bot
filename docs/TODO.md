...
- ~~`/storyadmin skip` / `close` / `pause` have no confirmation step~~ — **withdrawn 2026-09-22, the item was wrong.** Those three subcommands do not exist and never have (`commands/storyadmin.js` defines only `user`, `delete`, `sweep`, `setup`, `help`, `faqsync`; `git log -S` finds no history of the others). The real behaviour: skip and reassign live on the `/story manage` panel and DO confirm (`story/_manageTurnActions.js:68-73`, `:142-150`, Danger-styled confirm plus Cancel); `/story close` confirms (`story/close.js:71-89`); pause/resume are deliberately immediate (`story/_managePauseResume.js`) and are safely reversible — pause cancels the turn's pending jobs and locks the thread, resume unlocks it, retitles it, posts a notice and grants a **fresh full turn length** rather than the remainder. Root cause of the bad item: `docs/reference/ux_roadmap.md` documented three commands that were never built, and the 2026-08-21 entry-point audit read the roadmap rather than the code. Roadmap corrected in the same commit.
