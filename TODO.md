...
- Fable Audit fixes — see [docs/Fable_Audit_2026-07.md](docs/Fable_Audit_2026-07.md) "Suggested Fix Order" and [docs/Fable_Audit_Fix_Progress.md](docs/Fable_Audit_Fix_Progress.md) for full detail on what steps 1-3 actually touched.
  ...
  - Step 5 (docs/tests/constants module) still pending — see docs/Fable_Audit_2026-07.md's Suggested Fix Order.
  - **2026-07-12 sequencing decision:** step 6 (Tier-3 cleanup + Bucket 2 efficiency items) is not a separate scheduled session. The session doing step 5 should actively check, for each file it opens for step 5's own reasons (constants module, dead-code deletion, etc.), whether that file also has an open Tier-3/Bucket-2 finding — and fold it in if so, the same way step 4 incidentally closed **1.20** (close.js's missing job-cancellation on close — `closeStoryInternals` fixed this as a side effect of the 1.42 work, even though 1.20 wasn't on step 4's target list). After step 5, reassess what Tier-3/Bucket-2 items are still untouched and decide then whether they're worth a dedicated pass or stay opportunistic. **Confirmed still open as of this session** (files were touched by step 4 for other reasons, these specific issues were not): 1.15 (`_writeSkip.js` `handleThreadDeleteNow` has no owner/admin auth check), 1.18 (`_managePauseResume.js` hardcoded `'PAUSED'` + broken `[turnEndTime]` token replace — pause thread retitle silently does nothing), 1.54 (`_manageUser.js` inline `.replace()` instead of `replaceTemplateVariables`, `read.js` `log()` misuse), 1.23 (message-fetch caps in `_writeFinalize.js`/`_turn.js`), 2.4 (job-runner throughput/context-rebuild).
