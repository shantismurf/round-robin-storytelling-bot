...
- Fable Audit fixes — see [docs/Fable_Audit_2026-07.md](docs/Fable_Audit_2026-07.md) "Suggested Fix Order" and [docs/Fable_Audit_Fix_Progress.md](docs/Fable_Audit_Fix_Progress.md) for full detail on what steps 1-3 actually touched.
  ...
  - **2026-07-12 sequencing decision:** 1.7 and 1.17 both touch the same turn-end/thread-disposal/job-cancellation surface that step 4 is already opening up for `departWriter`/`closeStoryInternals` — fold both into step 4 rather than reopening those files in a separate session. 1.14 (join capacity race) lives in unrelated code (`join.js`'s transaction) and doesn't depend on step 4 — fine as a standalone fix any time.
