...
('txtSetupRequiredAdmin', 'Welcome to the Round Robin StoryBot! Before you can start writing collaborative stories with your community, you''ll need to create a channel that you want the stories to post in, and run `/storyadmin setup` to add it to the system.\n\nThat''s the only requirement, but there are other things you may want to set up first:\n\n* a **Media/Image channel** enables image embeds in longer story entries (best if it''s a private channel),\n* a **Story Admin role** defines who can manage all of your stories and writers, and\n* a **Story Writer role** defines who is interested in hearing about story activity.\n\nThese and other server setup options can be edited at any time, as needed. If you have any questions, please join the [Round Robin Storybot Hub server]([hubInviteUrl]). I''m happy to help!', 'en', 1),
('txtSetupRequiredUser', '**Round Robin StoryBot** runs collaborative stories — writers take turns adding to a shared story, one entry at a time.\n\nIt isn''t set up on this server yet. A server admin can finish that with `/storyadmin setup`, and then you''ll be able to join or start a story.', 'en', 1),
...
('txtSetupBotPermsFix', '_## To fix: Open the channel''s settings → Permissions, add **[bot_role_name]** under Roles/Members, and turn on the permissions listed above. This is required for the bot to have access to this channel._', 'en', 1),
