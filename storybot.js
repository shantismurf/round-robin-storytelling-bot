  ...
  try {
    ...
    const [storyResult] = await txn.execute(
      `INSERT INTO story (guild_id, guild_story_id, title, story_status, mode, turn_length_hours,
       ...
       rating, warnings, main_pairing, other_relationships, characters, dynamic, tags, scene_break_divider, summary, ground_rules)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        guild_id,
        nextGuildStoryId,
        ...
        storyInput.summary ?? null,
        storyInput.groundRules ?? null
      ]
    );
